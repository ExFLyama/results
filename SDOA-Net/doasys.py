...
def gen_refsp(doa, doa_grid, sigma):
    ...
    for i in range(doa.shape[1]):
        ...
        if not np.any(valid):
            continue
        dist = np.abs(doa_grid[None, :] - doa[valid, i][:, None])
        ref_sp[valid] += np.exp(- dist ** 2 / sigma ** 2)
    return ref_sp


def noise_torch(s, snr, fixed_snr=False):
    bsz, _, signal_dim = s.size()
    s = s.view(bsz, -1)
    sigma_max = np.sqrt(1. / snr)
    if fixed_snr:
        sigmas = torch.full((bsz,), sigma_max, device=s.device, dtype=s.dtype)
    else:
...
def get_doa(sp, doa_num, doa_grid, max_target_num, ref_doa):
    ...
    for n in range(len(doa_num)):
        ...
        peaks = np.sort(doa_grid[find_peaks_out[0][idx]])
        valid_ref = ref_doa[n] > -90
        for idx_tmp in np.where(valid_ref)[0]:
            est_doa[n, idx_tmp] = peaks[np.argmin(np.abs(ref_doa[n, idx_tmp] - peaks))]
...
def train_net(args, net, optimizer, criterion, train_loader, val_loader,
    ...
    for batch_idx, (clean_signal, target_sp, doa) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output_net = net(noisy_signal).view(noisy_signal.size(0), 2, -1)
        if net_type == 0:
